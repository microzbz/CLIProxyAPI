...
func defaultRequestLoggerFactory(cfg *config.Config, configPath string) logging.RequestLogger {
	...
	return logging.NewFileRequestLogger(cfg.RequestLog, logsDir, configDir, cfg.ErrorLogsMaxFiles, cfg.RequestLogRetentionDays)
}
...
func (s *Server) registerManagementRoutes() {
	...
	mgmt.Use(s.managementAvailabilityMiddleware(), s.mgmt.Middleware())
	{
		...
		mgmt.PATCH("/request-log-retention-days", s.mgmt.PutRequestLogRetentionDays)
		...
		mgmt.GET("/auth-rate-limit", s.mgmt.GetAuthRateLimit)
		mgmt.PUT("/auth-rate-limit", s.mgmt.PutAuthRateLimit)
		mgmt.PATCH("/auth-rate-limit", s.mgmt.PutAuthRateLimit)
		mgmt.GET("/auth-rate-limit/limit", s.mgmt.GetAuthRateLimitLimit)
		mgmt.PUT("/auth-rate-limit/limit", s.mgmt.PutAuthRateLimitLimit)
		mgmt.PATCH("/auth-rate-limit/limit", s.mgmt.PutAuthRateLimitLimit)
		mgmt.GET("/auth-rate-limit/window-seconds", s.mgmt.GetAuthRateLimitWindowSeconds)
		mgmt.PUT("/auth-rate-limit/window-seconds", s.mgmt.PutAuthRateLimitWindowSeconds)
		mgmt.PATCH("/auth-rate-limit/window-seconds", s.mgmt.PutAuthRateLimitWindowSeconds)
		mgmt.GET("/auth-rate-limit/cooldown-seconds", s.mgmt.GetAuthRateLimitCooldownSeconds)
		mgmt.PUT("/auth-rate-limit/cooldown-seconds", s.mgmt.PutAuthRateLimitCooldownSeconds)
		mgmt.PATCH("/auth-rate-limit/cooldown-seconds", s.mgmt.PutAuthRateLimitCooldownSeconds)
...
func injectAuthAccessTokenEditorHTML(html string) string {
	if strings.Contains(html, "cpa-auth-access-token-editor-v5") || !strings.Contains(html, "</body>") {
		return html
	}
	const patch = `<script>
(function(){
  const PATCH_MARKER="cpa-auth-access-token-editor-v5";
  ...
  let directSaveInFlight=null;
  document.documentElement.setAttribute("data-cpa-auth-access-token-editor",PATCH_MARKER);
  function clean(value){return String(value||"").replace(/\s+/g," ").trim()}
  function norm(value){const raw=String(value||"").trim();const m=raw.match(/^Bearer\s+(.+)$/i);return m?m[1].trim():raw}
  function storage(storage,key){try{return storage&&storage.getItem?storage.getItem(key):""}catch(_){return ""}}
  function findManagementKeyInObject(value){if(!value||typeof value!=="object")return "";if(typeof value.managementKey==="string")return value.managementKey;for(const k of Object.keys(value)){const found=findManagementKeyInObject(value[k]);if(found)return found}return ""}
  function scanStorageForManagementKey(storageObj){try{if(!storageObj||typeof storageObj.length!=="number")return "";for(let i=0;i<storageObj.length;i++){const entryKey=storageObj.key(i);if(!entryKey)continue;const raw=storageObj.getItem(entryKey);if(!raw||raw.indexOf("managementKey")===-1)continue;try{const found=findManagementKeyInObject(JSON.parse(raw));if(found)return found}catch(_){}}}catch(_){}return ""}
  function cacheKey(value){const k=norm(value);if(!k)return "";try{window.__CPA_AUTH_RATE_LIMIT_MANAGEMENT_KEY__=k}catch(_){}try{sessionStorage.setItem("cpaAuthRateLimitManagementKey",k)}catch(_){}return k}
  function key(){const direct=norm(window.__CPA_AUTH_RATE_LIMIT_MANAGEMENT_KEY__)||norm(storage(localStorage,"managementKey"))||norm(storage(sessionStorage,"managementKey"))||norm(storage(sessionStorage,"cpaAuthRateLimitManagementKey"))||norm(storage(localStorage,"cpaAuthRateLimitManagementKey"));if(direct)return cacheKey(direct);const scanned=norm(scanStorageForManagementKey(localStorage))||norm(scanStorageForManagementKey(sessionStorage));return scanned?cacheKey(scanned):""}
  function headers(extra){const h=new Headers(extra||{});const k=key();if(k&&!h.has("Authorization"))h.set("Authorization","Bearer "+k);return h}
  ...
  function looksLikeDialog(el){if(!el||!visible(el))return false;const value=clean(el.textContent);return /认证文件详情\s*\/\s*编辑|Auth File.*Edit|Auth File.*Details/i.test(value)&&/access_token|accessToken|认证文件 JSON/i.test(value)}
  function dialog(){const selectors=['[role="dialog"]','[aria-modal="true"]','.modal','.dialog','[class*="modal" i]','[class*="dialog" i]'];let nodes=[];selectors.forEach(function(selector){try{nodes=nodes.concat(Array.prototype.slice.call(document.querySelectorAll(selector)))}catch(_){}});let matched=nodes.filter(looksLikeDialog);if(matched.length===0){matched=Array.prototype.slice.call(document.querySelectorAll("body *")).filter(function(el){if(!looksLikeDialog(el))return false;const parent=el.parentElement;return !parent||!looksLikeDialog(parent)})}matched.sort(function(a,b){return clean(a.textContent).length-clean(b.textContent).length});return matched[0]||null}
  ...
  function updateSaveButton(){if(!tokenChanged())return;const b=saveButton();if(b){b.disabled=false;b.removeAttribute("disabled");b.setAttribute("aria-disabled","false");b.style.pointerEvents="auto"}}
  ...
  async function fallbackSave(){const token=currentToken();const name=authName();if(!token.exists||!tokenChanged()||!name)return;if(directSaveInFlight)return directSaveInFlight;directSaveInFlight=(async function(){const response=await fetch(ENDPOINT,{method:"PATCH",headers:headers({"Content-Type":"application/json"}),body:JSON.stringify({name:name,access_token:token.value})});if(!response.ok){const payload=await response.json().catch(function(){return {}});throw new Error(payload.error||payload.message||response.statusText||"request failed")}markSaved()})();try{return await directSaveInFlight}finally{directSaveInFlight=null}}
  ...
  document.addEventListener("click",function(event){const b=event.target&&event.target.closest?event.target.closest("button"):null;if(!b||b!==saveButton()||!tokenChanged())return;const started=Date.now();setTimeout(function(){if(lastAugmentedAt>=started){markSaved();return}fallbackSave().then(function(){const btn=saveButton();if(btn){const old=btn.textContent;btn.textContent="已保存";setTimeout(function(){btn.textContent=old},1000)}}).catch(function(err){const btn=saveButton();if(btn)btn.textContent="保存失败";console.error(err)})},350)},true);
...
func (s *Server) UpdateClients(cfg *config.Config) {
	...
	if s.requestLogger != nil && (oldCfg == nil || oldCfg.RequestLogRetentionDays != cfg.RequestLogRetentionDays) {
		if setter, ok := s.requestLogger.(interface{ SetRequestLogRetentionDays(int) }); ok {
			setter.SetRequestLogRetentionDays(cfg.RequestLogRetentionDays)
		}
	}
